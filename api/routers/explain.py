...
class ExplainResponse(BaseModel):
    ...
    fraud_probability: float = Field(
        description="Model probability that this transaction is fraud.")
    risk_level: str = Field(description="Risk tier: LOW / MEDIUM / HIGH / CRITICAL.")
    predicted_label: int = Field(
        description="Hard label at the F1-optimal threshold: 1=fraud, 0=legitimate.")
    scored_at: str | None = Field(
        description="ISO-8601 UTC timestamp of original scoring, if available.")
...
class FeatureImportanceItem(BaseModel):
    ...
    importance: float = Field(
        description="Normalised importance score in [0, 1]. All scores sum to 1.")
